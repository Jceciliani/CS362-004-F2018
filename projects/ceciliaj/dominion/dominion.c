...
int smithyCard(struct gameState *state, int currentPlayer, int handPos)
{
      //+3 Cards
      int i;
      for (i = 0; i < 4; i++) // Bug - Drawing 4 cards instead of 3
	{
	  drawCard(currentPlayer, state);
	}
...
int villageCard(struct gameState *state, int currentPlayer, int handPos)
{
      ...
      //+2 Actions
      state->numActions = state->numActions + 3; // Bug - +3 actions instead of +2
...
int greatHallCard(struct gameState *state, int currentPlayer, int handPos)
{
      ...
      //+1 Actions
      state->numActions--; // Bug - Should be "state->numActions++;" - Maybe cause player to lose all actions for the turn/potential seg fault upon playing this card
